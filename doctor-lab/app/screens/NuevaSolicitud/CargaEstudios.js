...
export default function CargaEstudios(props) {
  ...
  const onSubmit = async () => {
   ...
   // console.log(estudiosSelected+"<----");

    //console.log(`http://procureltg.com/Pruebas/LabTellezGiron/APIDoctoresAltaSolicitud.php?json=[{"idPaciente":"${paciente.idPaciente}","UTM":"${paciente.UTM}","email":"${paciente.email}","telefono":"${paciente.telefono}","idUsuario":"${usTemp}","fechaProp":" ","indicaciones":" ","estudios":[${estudiosSelected}]}]`);
    //console.log(estudiosSelected.length);
    if(estudiosSelected.length===0){
      toastRef.current.show("Tiene que tener estudios agregados para finalizar la solicitud");
    }else{
    setLoading(true);
    await fetch(`http://procureltg.com/Pruebas/LabTellezGiron/APIDoctoresAltaSolicitud.php?json=[{"idPaciente":"${paciente.idPaciente}","UTM":"${paciente.UTM}","email":"${paciente.email}","telefono":"${paciente.telefono}","idUsuario":"${usTemp}","fechaProp":" ","indicaciones":" ","estudios":[${estudiosSelected}]}]`)
  ...
  const onSubmitBusqueda = async () => {
    ...
    if (size(formData.estudioB) == 0) {
      ...
      setLoading(true);
      await fetch(`http://procureltg.com/Pruebas/APIAppDoctores/APIDoctoresBusquedaEstudios.php?Estudio=${formData.estudioB}`)
        .then((response) => response.json())
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`http://procureltg.com/Pruebas/APIAppDoctores/APIDoctoresEstudiosBasicos.php`)
      .then((response) => response.json())
