...
export default function NuevoPacienteForm(props) {
    ...
    const onSubmit = async () => {
         console.log(formData);
         console.log( formData.fechaNacimiento===null);
        if (isEmpty(formData.paterno) || isEmpty(formData.materno) || isEmpty(formData.nombre)
            || formData.sexo===" " || formData.fechaNacimiento==="") {
            ...
            const response = await fetch(
                `http://procureltg.com/Pruebas/APIAppDoctores/APIDoctoresAltaPaciente.php?ApPaterno=${formData.paterno}&ApMaterno=${formData.materno}&Nombre=${formData.nombre}&Sexo=${formData.sexo}&FechaNac=${convert(formData.fechaNacimiento)}&Mail=${formData.email}&Tel=${formData.telefono}`
            );
