...
export default function DatosSolicitudForm(props) {
  ...
  const enviarResultados = async () => {
    if (size(formData.telefono != 10)) {
      ...
      await fetch(
        `https://ltgprocure.com/LabTellezGiron/APIDoctoresImpresionResultados.php?IdFolio=${formData.IdFolio}&EmailNuevoUsuario=${formData.email}&Telefono=${formData.telefono}`
      )
  ...
  const consultaPDFResultados = async () => {
    ...
    const response = await fetch(
      `https://ltgprocure.com/LabTellezGiron/APIDoctoresConsultaDeResultados.php?IdFolio=${formData.IdFolio}`
    );
